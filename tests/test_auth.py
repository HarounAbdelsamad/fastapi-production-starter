...
async def test_logout_revokes_access(client):
    ...
    protected_resp = await client.get("/api/users/", headers=headers)
...
async def test_rbac_admin_guard(client):
    ...
    forbidden = await client.get("/api/admin/dashboard", headers=user_headers)
    assert forbidden.status_code == 403

    admin_payload = make_user_payload(role="admin")
    await client.post("/api/users/", json=admin_payload)
    ...
    allowed = await client.get("/api/admin/dashboard", headers=admin_headers)
